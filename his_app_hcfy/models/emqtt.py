...
class Emqtt(models.TransientModel):
    ...
    def on_connect(cls, client, rc):
        ...
        if rc == 0:
            client.subscribe([(cls.topic, 0), ('public', 0)])  # internet 外网  intranet 内网
        ...
        try:
            ...
            if message:
                if not EmqttMessageProcess.get('emqtt'):
                    EmqttMessageProcess['emqtt'] = importlib.import_module('odoo.addons.his_app_hcfy.models.emqtt_message_process')
    ...
    def process_message_callback(self, data):
        try:
            ...
            if source_topic != config['extranet_topic']:
                app_token = self.env['his.app_token'].sudo().search([('mac', '=', mac)], order='id desc', limit=1)
                if not app_token:
            ...
            base_data_message_obj = self.env['his.base_data_message'].sudo()
            ...
            # # 将消息保存到数据库中
            # company_id = False
            # company = self.env['res.company'].search([('topic', '=', source_topic)])
            # if company:
