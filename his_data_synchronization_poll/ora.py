...
class Ora(object):
    ...
    def __getConn():
        ...
        if Ora.__pool is None:
            Ora.__pool = PooledDB(cx_Oracle, user=config['orcl_username'], password=config['orcl_pwd'], dsn="%s:1521/%s" % (config['orcl_ip'], config['orcl_instance']),  mincached=5, maxcached=20)
        return Ora.__pool.connection()
    ...
    # 根据表自动创建参数字典
    def create_params(self, table, args=None):
        if args is None:
            args = {}
    ...
    def query(self, sql, args=None, name_en=None):
        if args is None:
            args = {}
        if name_en:
            _logger.info(u'正在查询Oracle')
        self.execute(sql, args)
        if name_en:
            _logger.info(u'结束查询Oracle')
        res = self.get_rows()
        if name_en:
            _logger.info(u'格式化返回数据完成')
        return res
